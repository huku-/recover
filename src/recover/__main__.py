...
from recover.exporter import Data, Segment
from recover.graphs import AFCG, DFG, PDG, EdgeType, EdgeClass, NodeType # pylint: disable=unused-import
...
def _get_segment_selector(data: Data, name: str) -> Optional[int]:
...
def main(argv: List[str]) -> int:
    ...
    parser.add_argument('--optimizer', '-o',
        choices=['brute', 'genetic', 'mixed'], type=str, default='brute',
        help='algorithm to use for compile-unit layout optimization')
    parser.add_argument('--filter-method', '-t',
        choices=['none', 'rank'], type=str, default='none',
        help='graph filtering method to use before estimation and optimization')
    parser.add_argument('--segment', '-s', metavar='NAME',
    ...
    parser.add_argument('output_file', metavar='FILE', type=pathlib.Path,
    ...
    with open(f'{args.output_file}.time', 'w') as fp:
